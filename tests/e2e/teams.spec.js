...
test.describe("Team Member Management", () => {
  ...
  test("cannot add user already in a team", async ({ request }) => {
    ...
    const addRes = await request.post(
      `${BASE_URL}/api/teams/${teamId1}/members`,
      {
        headers: authHeaders(owner1.token),
  ...
  test("owner can remove members", async ({ request }) => {
    ...
    const removeRes = await request.delete(
      `${BASE_URL}/api/teams/${teamId}/members/${member.userId}`,
      {
        headers: authHeaders(owner.token),
      },
    );
  ...
  test("member can leave team", async ({ request }) => {
    ...
    const leaveRes = await request.post(
      `${BASE_URL}/api/teams/${teamId}/leave`,
      {
        headers: authHeaders(member.token),
      },
    );
  ...
  test("cannot remove team owner", async ({ request }) => {
    ...
    const removeRes = await request.delete(
      `${BASE_URL}/api/teams/${teamId}/members/${owner.userId}`,
      {
        headers: authHeaders(owner.token),
      },
    );
...
test.describe("Team Workspace Visibility", () => {
  test("team members see each other projects in team mode", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    const owner = await createApprovedUser(request, adminToken, "own");
  ...
  test("team mode does not allow cross-team writes", async ({ request }) => {
    ...
    // Owner creates a project
    const { body: project } = await createProjectAPI(request, owner.token, {
      title: "Read Only",
    });
...
test.describe("UI: Workspace Toggle", () => {
  test("workspace toggle is visible after login", async ({ page }) => {
    ...
    await expect(page.locator("#workspaceToggle")).toBeVisible({
      timeout: 5000,
    });
    await expect(
      page.locator('.workspace-toggle-btn[data-mode="team"]'),
    ).toBeVisible();
    await expect(
      page.locator('.workspace-toggle-btn[data-mode="personal"]'),
    ).toBeVisible();
  });
  ...
  test("team project cards show creator usernames", async ({
    request,
    page,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    ...
    await createProjectAPI(request, owner.token, {
      title: "Owner Label Project",
    });
    await createProjectAPI(request, member.token, {
      title: "Member Label Project",
    });
    await request.post(`${BASE_URL}/api/settings/workspaceMode`, {
      headers: authHeaders(member.token),
      data: { value: "team" },
