...
async function createApprovedUser(request, adminToken, prefix = "user") {
  ...
  const token = extractSessionTokenFromHeaders(loginRes.headers());
...
test.describe("Red-Team Fixes", () => {
  test("tag payload is rendered as data, not executable attributes", async ({
    request,
    page,
  }) => {
  ...
  test("rapid create-project clicks only create one project", async ({
    page,
  }) => {
    await loginUI(page);
    await page.waitForSelector("#newProject");
  ...
  test("workspace toggle settles on the latest selection after fast clicks", async ({
    request,
    page,
  }) => {
    ...
    await createProjectAPI(request, owner.token, {
      title: "owner-team-project",
    });
    await createProjectAPI(request, member.token, {
      title: "member-team-project",
    });
    ...
    await expect
      .poll(async () => {
        const activeMode = await page
          .locator(".workspace-toggle-btn.active")
  ...
  test("rapid create-team clicks only submit once from the UI", async ({
    request,
    page,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    ...
    await page.fill(
      '#teamContent input[placeholder="Team name"]',
      "Rapid Team",
    );
    await page.evaluate(() => {
      const createBtn = Array.from(
        document.querySelectorAll("#teamContent button"),
  ...
  test("concurrent create-team requests cannot create multiple teams for the same user", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    const owner = await createApprovedUser(
  ...
  test("concurrent invites cannot add the same user to multiple teams", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    ...
    const statuses = inviteResponses
      .map((response) => response.status())
      .sort((a, b) => a - b);
