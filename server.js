...
try {
  ...
  logger.warn(
    "mammoth not installed — docx previews will fall back to download only",
  );
}
...
if (process.env.NODE_ENV === "production" && !APP_ORIGIN) {
  throw new Error(
    "APP_ORIGIN must be set in production, or VERCEL_PROJECT_PRODUCTION_URL must be available",
  );
}

if (process.env.NODE_ENV === "production" && trustProxy === true) {
  throw new Error(
    "TRUST_PROXY=true is too broad in production; set TRUST_PROXY to the actual proxy hop count or subnet",
  );
}
...
if (process.env.DISABLE_RATE_LIMIT !== "1") {
  ...
  try {
    ...
    logger.warn(
      "express-rate-limit not installed — running without rate limiting",
    );
  }
}
...
function injectAssetUrls(html) {
  ...
  return html
    ...
    .replaceAll(
      "/dist/boot.bundle.js",
      `/dist/${bundleMap["boot.bundle.js"] || "boot.bundle.js"}`,
    )
    .replaceAll(
      "/dist/app-shell.bundle.js",
      `/dist/${bundleMap["app-shell.bundle.js"] || "app-shell.bundle.js"}`,
    )
    .replaceAll(
      "/dist/app.bundle.js",
      `/dist/${bundleMap["app.bundle.js"] || "app.bundle.js"}`,
    )
    .replaceAll(
      "/dist/admin.bundle.js",
      `/dist/${bundleMap["admin.bundle.js"] || "admin.bundle.js"}`,
    )
    .replaceAll(
      "/dist/login.bundle.js",
      `/dist/${bundleMap["login.bundle.js"] || "login.bundle.js"}`,
    )
    .replaceAll(
      "/dist/register.bundle.js",
      `/dist/${bundleMap["register.bundle.js"] || "register.bundle.js"}`,
    );
}
...
if (z.object && z.string && typeof z.string === "function") {
  try {
    ...
    const emailDateSchema = z.union([
      z.string().regex(DATE_INPUT_REGEX).max(10),
      z.literal(""),
      z.null(),
    ]);
    ...
    const importDocxDocumentSchema = z.object({
      ...
      mimeType: z
        .string()
        .refine(
          (value) => ALLOWED_MIME_TYPES.has(value),
    ...
    schemas.adminResetPassword = z.object({
      adminPassword: z.string().min(1).max(MAX_PASSWORD_LENGTH),
    ...
    schemas.createTaskBulk = z.array(
      z.object({
...
async function requireAuth(req, res, next) {
  try {
    ...
    if (sessionLookup.status !== "ok" || !sessionLookup.session) {
      ...
      logSecurityEvent(
        `auth.session.${sessionLookup.status === "not_found" ? "invalid" : sessionLookup.status}`,
        {
          req,
          statusCode: 401,
...
function requireSameOriginCookieWrite(req, res, next) {
  ...
  const sourceOrigin =
    parseHeaderOrigin(req.headers.origin) ||
...
async function enforceMaintenanceMode(req, res) {
  ...
  if (requestPath.startsWith("/api/")) {
    ...
    res
      .status(503)
      .type("text/plain")
      .send("Service unavailable for maintenance");
  }
  return true;
}
...
function getSafeDocumentFileName(document) {
  const fallback =
    document.mimeType === "application/pdf" ? "document.pdf" : "document.docx";
...
app.use(async (req, res, next) => {
  try {
    ...
    res
      .status(503)
      .json({ error: "Service unavailable — initialization failed" });
  }
});
...
const { projectTasksRouter, tasksRouter } = createTasksRouters({
  db,
  logger,
  schemas,
  requireAuth,
...
const exportImportRouter = createExportImportRouter({
  db,
  logger,
  schemas,
  requireAuth,
...
async function seedAdminUser() {
  ...
  if (!adminUser || !adminPass) {
    logger.warn(
      "ADMIN_USER and ADMIN_PASS env vars not set — skipping admin seed",
    );
    return;
  }
  ...
  if (!policyResult.valid) {
    throw new Error(
      `ADMIN_PASS rejected by password policy: ${policyResult.message}`,
    );
  }
  ...
  console.warn(
    "[SECURITY] Admin user seeded. REMOVE ADMIN_PASS from your environment immediately. It is no longer needed after initial setup.",
  );
}
...
async function startServer() {
  ...
  try {
    ...
    logger.warn(
      { err: err.message },
      "WebSocket server not available (ws module missing or error)",
    );
  }
