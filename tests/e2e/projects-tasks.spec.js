...
test.describe("Projects & Tasks CRUD", () => {
  ...
  test("get single project", async ({ request }) => {
    const { token } = await loginAPI(request);
    const { body: created } = await createProjectAPI(request, token, {
      title: "Single Get",
    });
  ...
  test("delete a project", async ({ request }) => {
    ...
    const delRes = await request.delete(
      `${BASE_URL}/api/projects/${created.id}`,
      {
        headers: authHeaders(token),
      },
    );
  ...
  test("preview email and document attachments", async ({ request }) => {
    ...
    const { response: emailRes, body: emailBody } = await createDocumentAPI(
      request,
      token,
      project.id,
      {
    ...
    const emailPreviewRes = await request.get(
      `${BASE_URL}/api/documents/${emailBody.id}/preview`,
      {
        headers: authHeaders(token),
      },
    );
    ...
    const { response: docRes, body: docBody } = await createDocumentAPI(
      request,
      token,
      project.id,
      {
    ...
    const docPreviewRes = await request.get(
      `${BASE_URL}/api/documents/${docBody.id}/preview`,
      {
        headers: authHeaders(token),
      },
    );
  ...
  test("reject task for non-existent project", async ({ request }) => {
    const { token } = await loginAPI(request);
    const res = await request.post(
      `${BASE_URL}/api/projects/00000000-0000-0000-0000-000000000000/tasks`,
      {
        headers: authHeaders(token),
  ...
  test("open document renders inline preview in the project view", async ({
    request,
    page,
  }) => {
    const { token } = await loginAPI(request);
    ...
    const openButton = page
      .locator('.doc-item [data-doc-action="open"]')
      .first();
  ...
  test("document persists when a stale poll resolves after attachment and sidebar navigation", async ({
    request,
    page,
  }) => {
    const { token } = await loginAPI(request);
    ...
    await expect(page.locator("#projectHomeDocuments .doc-item")).toHaveCount(
      1,
    );
    await expect(page.locator("#projectHomeDocuments")).toContainText(
      "fixture.docx",
    );
...
test.describe("Data Isolation", () => {
  async function setupTwoUsers(request) {
    ...
    const { body: regA } = await registerAPI(request, {
      username: userA,
      email: `${userA}@test.com`,
      password: "SecurePass123",
    });
    const { body: regB } = await registerAPI(request, {
      username: userB,
    ...
    const loginA = await request.post(`${BASE_URL}/api/auth/login`, {
      data: { username: userA, password: "SecurePass123" },
    });
    const tokenA = extractSessionTokenFromHeaders(loginA.headers());
    const loginB = await request.post(`${BASE_URL}/api/auth/login`, {
      data: { username: userB, password: "SecurePass123" },
    });
    const tokenB = extractSessionTokenFromHeaders(loginB.headers());
  ...
  test("user A cannot see user B projects", async ({ request }) => {
    ...
    const { body: projectA } = await createProjectAPI(request, tokenA, {
      title: "User A Secret",
    });
    ...
    const directRes = await request.get(
      `${BASE_URL}/api/projects/${projectA.id}`,
      {
        headers: authHeaders(tokenB),
      },
    );
