...
const SECURITY_LOG_PATH = path.resolve(
  __dirname,
...
function readSecurityLogEntries() {
  ...
  return fs
    .readFileSync(SECURITY_LOG_PATH, "utf8")
    .split("\n")
    .filter(Boolean)
...
async function createApprovedUserToken(request, prefix = "security") {
  ...
  return extractSessionTokenFromHeaders(loginRes.headers());
}
...
test.describe("Security Headers & Input Validation", () => {
  ...
  test("reject invalid global settings key", async ({ request }) => {
    ...
    const res = await request.post(
      `${BASE_URL}/api/admin/settings/maliciousKey`,
      {
        headers: authHeaders(token),
        data: { value: "evil" },
      },
  ...
  test("reject document upload with mismatched allowed mime and file signature", async ({
    request,
  }) => {
    const { token } = await loginAPI(request);
    const { body: project } = await createProjectAPI(request, token, {
  ...
  test("user settings CRUD", async ({ request }) => {
    ...
    expect(allRes.headers()["cache-control"]).toBe(
  ...
  test("same-origin cookie-authenticated writes succeed", async ({
    request,
  }) => {
    const loginRes = await request.post(`${BASE_URL}/api/auth/login`, {
  ...
  test("cross-site cookie-authenticated writes are rejected", async ({
    request,
  }) => {
    const loginRes = await request.post(`${BASE_URL}/api/auth/login`, {
      data: { username: "testadmin", password: "SecureTestPass123" },
  ...
  test("failed login is written to the security event log", async ({
    request,
  }) => {
    ...
    const events = readSecurityLogEntries().filter(
      (entry) => entry.eventType === "auth.login.failed",
    );
    const loginFailure = events.find(
      (entry) => entry.attemptedUsername === attemptedUsername,
    );
  ...
  test("admin authorization denials are written to the security event log", async ({
    request,
  }) => {
    fs.rmSync(SECURITY_LOG_PATH, { force: true });
    ...
    await request.put(
      `${BASE_URL}/api/admin/users/${userBody.user.id}/approve`,
      {
        headers: authHeaders(token),
    ...
    const loginToken = extractSessionTokenFromHeaders(loginRes.headers());
    ...
    const events = readSecurityLogEntries().filter(
      (entry) => entry.eventType === "authz.admin_denied",
    );
    const denialEvent = events.find(
      (entry) => entry.actorUsername === userBody.user.username,
    );
  ...
  test("reject import with unsupported document MIME type", async ({
    request,
  }) => {
    const { token } = await loginAPI(request);
    const res = await request.post(`${BASE_URL}/api/import`, {
  ...
  test("reject import with mismatched document signature", async ({
    request,
  }) => {
    const { token } = await loginAPI(request);
    const res = await request.post(`${BASE_URL}/api/import`, {
  ...
  test("non-API routes serve index.html (SPA fallback)", async ({
    request,
  }) => {
