...
test.describe("Role-Based Access Control", () => {
  ...
  test("admin can change user role", async ({ request }) => {
    ...
    const res = await request.put(
      `${BASE_URL}/api/admin/users/${body.user.id}/role`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ role: "admin" }),
      },
  ...
  test("admin can delete a user", async ({ request }) => {
    ...
    const res = await request.delete(
      `${BASE_URL}/api/admin/users/${body.user.id}`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload(),
      },
  ...
  test("approve returns 404 for non-existent user", async ({ request }) => {
    const { adminToken } = await setupUserAndAdmin(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/non-existent-id/approve`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload(),
      },
  ...
  test("role change returns 404 for non-existent user", async ({ request }) => {
    const { adminToken } = await setupUserAndAdmin(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/non-existent-id/role`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ role: "admin" }),
      },
  ...
  test("full workflow: register → admin approves → user can login", async ({
    request,
  }) => {
    // 1. Admin logs in
    const { token: adminToken } = await loginAPI(request);
    ...
    const loginBeforeApproval = await request.post(
    ...
    const approveRes = await request.put(
      `${BASE_URL}/api/admin/users/${newUserId}/approve`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload(),
      },
    ...
    const loginAfterApproval = await request.post(
      `${BASE_URL}/api/auth/login`,
      {
        data: { username: newUser, password: "WorkflowPass123" },
      },
  ...
  test("full workflow: register → admin promotes to admin → user has admin access", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    ...
    const promoteRes = await request.put(
      `${BASE_URL}/api/admin/users/${regBody.user.id}/role`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ role: "admin" }),
      },
  ...
  test("full workflow: register → approve → delete → user cannot login", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    ...
    const deleteRes = await request.delete(
      `${BASE_URL}/api/admin/users/${regBody.user.id}`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload(),
      },
  ...
  test("admin panel: buttons are clickable and approve works via UI", async ({
    page,
    request,
  }) => {
    ...
    const userRow = pendingSection.locator(".admin-user-row", {
      hasText: newUser,
    });
    ...
    const approvedRow = allUsersList.locator(".admin-user-row", {
      hasText: newUser,
    });
  ...
  test("admin panel: delete button works via UI", async ({ page, request }) => {
    ...
    const userRow = page.locator("#userList .admin-user-row", {
      hasText: newUser,
    });
    await expect(userRow).toBeVisible();
  ...
  test("admin resets another user password, sessions killed, new password works", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    const username = uniqueUser("pwreset");
    const { body: regBody } = await registerAPI(request, {
      username,
      email: `${username}@test.com`,
      password: "OriginalUserPwd99",
    });
    const userId = regBody.user.id;
    await approveUserAPI(request, adminToken, userId);

    const userLogin = await request.post(`${BASE_URL}/api/auth/login`, {
      data: { username, password: "OriginalUserPwd99" },
    });
    const { token: userToken } = await userLogin.json();

    const newPassword = "RotatedSecret9876";
    const res = await request.put(
      `${BASE_URL}/api/admin/users/${userId}/password`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ newPassword }),
      },
    );
    expect(res.status()).toBe(200);

    // Target user's existing token is invalidated
    const meRes = await request.get(`${BASE_URL}/api/auth/me`, {
      headers: authHeaders(userToken),
    });
  ...
  test("non-admin cannot reset another user password", async ({ request }) => {
    const { userToken, userId } = await setupUserAndAdmin(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/${userId}/password`,
      {
        headers: authHeaders(userToken),
        data: adminStepUpPayload({ newPassword: "AnotherStrongPwd999" }),
      },
    );
    expect(res.status()).toBe(403);
  });

  test("admin reset password rejects wrong admin password", async ({
    request,
  }) => {
    const { adminToken, userId } = await setupUserAndAdmin(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/${userId}/password`,
      {
        headers: authHeaders(adminToken),
        data: {
          ...
          newPassword: "AnotherStrongPwd999",
        },
      },
    );
  ...
  test("admin reset password rejects weak new password", async ({
    request,
  }) => {
    const { adminToken, userId } = await setupUserAndAdmin(request);
    const res = await request.put(
      ...
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ newPassword: "short" }),
      },
    );
    expect(res.status()).toBe(400);
  });

  test("admin reset password rejects common password", async ({ request }) => {
    const { adminToken, userId } = await setupUserAndAdmin(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/${userId}/password`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ newPassword: "Password1234" }),
      },
    );
    expect(res.status()).toBe(400);
  });

  test("admin cannot reset their own password via admin route", async ({
    request,
  }) => {
    const { token: adminToken, user: adminUser } = await loginAPI(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/${adminUser.id}/password`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ newPassword: "AdminSelfResetBlocked99" }),
      },
    );
    expect(res.status()).toBe(400);
  });

  test("admin reset password returns 404 for unknown user", async ({
    request,
  }) => {
    const { token: adminToken } = await loginAPI(request);
    const res = await request.put(
      `${BASE_URL}/api/admin/users/00000000-0000-0000-0000-000000000000/password`,
      {
        headers: authHeaders(adminToken),
        data: adminStepUpPayload({ newPassword: "AnotherStrongPwd999" }),
      },
