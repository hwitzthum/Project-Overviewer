...
async function init() {
  ...
  if (
    ...
    typeof window.AccountSettings.init === "function"
  ) {
  ...
  document.querySelector(".sidebar-nav").addEventListener("click", (e) => {
    ...
    if (
      ...
      isAllowedView(navItem.dataset.view)
    ) {
      document
  ...
  document
    .getElementById("createProjectSubmit")
  ...
  document
    .getElementById("quickActionDocxInput")
  ...
  document
    .querySelectorAll("#settingsModal .settings-option[data-theme]")
    .forEach((option) => {
  ...
  document.getElementById("convertToTask").addEventListener("click", () => {
    ...
    const targetProject =
      (currentProjectId &&
        state.projects.find((p) => p.id === currentProjectId && !p.archived)) ||
      state.projects.find(
        (p) =>
          !p.archived &&
          (!currentWorkspaceMode ||
            currentWorkspaceMode === "personal" ||
