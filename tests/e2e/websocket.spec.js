...
async function loginForWS(request, creds = ADMIN) {
  ...
  // Extract session_token from Set-Cookie header (the login response no longer
  // includes the raw token in the JSON body — see security event 49933a1).
  const setCookies = res.headers()["set-cookie"] || "";
  const match = setCookies.match(/session_token=([^\s;]+)/);
  const cookieToken = match ? match[1] : null;
  return { token: cookieToken, cookieToken, user: body.user };
}
...
test.describe("WebSocket — Real-time notifications", () => {
  ...
  test("project.created event is received on WebSocket after creating a project", async ({
    request,
  }) => {
    ...
    try {
      ...
      await createProjectAPI(request, adminLogin.token, {
  ...
  test("task.created event is received after adding a task", async ({
    request,
  }) => {
    const ws = await connectWS(adminLogin.cookieToken);
    try {
      // Create a project first
      const { body: project } = await createProjectAPI(
        request,
        adminLogin.token,
        { title: "WS Task Test" },
      );
  ...
  test("project.updated event is received after modifying a project", async ({
    request,
  }) => {
    const ws = await connectWS(adminLogin.cookieToken);
    try {
  ...
  test("project.deleted event is received after deleting a project", async ({
    request,
  }) => {
    const ws = await connectWS(adminLogin.cookieToken);
    try {
  ...
  test("task.updated event is received after toggling a task", async ({
    request,
  }) => {
    const ws = await connectWS(adminLogin.cookieToken);
    try {
  ...
  test("task.deleted event is received after removing a task", async ({
    request,
  }) => {
    const ws = await connectWS(adminLogin.cookieToken);
    try {
...
test.describe("WebSocket — Scoped broadcasts", () => {
  test("team member receives events from teammate, non-team user does not", async ({
    request,
  }) => {
    ...
    // Approve users
    const users = await (
      await request.get(`${BASE_URL}/api/admin/users`, {
        headers: authHeaders(adminLogin.token),
      })
    ).json();
    ...
    const memberLogin = await loginForWS(request, {
      username: memberName,
      password: "SecurePass12345",
    });
    const outsiderLogin = await loginForWS(request, {
      username: outsiderName,
      password: "SecurePass12345",
    });
    ...
    const team = await (
      await request.get(`${BASE_URL}/api/teams/mine`, {
        headers: authHeaders(adminLogin.token),
      })
    ).json();
    ...
    try {
      ...
      await request.delete(
        `${BASE_URL}/api/teams/${team.id}/members/${memberUser.id}`,
        {
          headers: authHeaders(adminLogin.token),
        },
      );
...
test.describe("WebSocket — Connection lifecycle", () => {
  ...
  test("multiple simultaneous connections from same user both receive events", async ({
    request,
  }) => {
    const adminLogin = await loginForWS(request);
