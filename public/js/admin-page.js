...
function renderAdminStats() {
  ...
  document.getElementById("statPending").textContent = adminUsers.filter(
    (user) => !user.approved,
  ).length;
  document.getElementById("statActive").textContent = adminUsers.filter(
    (user) => user.approved,
  ).length;
}
...
function renderAdminUserRow(user) {
  ...
  roleBtn.addEventListener("click", () =>
    changeAdminUserRole(user.id, user.role === "admin" ? "user" : "admin"),
  );
  actions.appendChild(roleBtn);

  if (user.approved) {
    const resetPwBtn = document.createElement("button");
    resetPwBtn.className = "admin-btn";
    resetPwBtn.textContent = "Reset Password";
    resetPwBtn.addEventListener("click", () =>
      resetAdminUserPassword(user.id, user.username),
    );
  ...
  deleteBtn.addEventListener("click", () =>
    deleteAdminUser(user.id, user.username),
  );
...
function openAdminModal({
  ...
  return new Promise((resolve) => {
    ...
    const newPwWrap = document.getElementById("adminModalNewPasswordWrap");
    const newPwInput = document.getElementById("adminModalNewPassword");
    const newPwError = document.getElementById("adminModalNewPasswordError");
    ...
    if (newPwWrap) {
      newPwWrap.style.display = withNewPasswordField ? "" : "none";
      if (withNewPasswordField) {
        newPwInput.value = "";
    ...
    document
      ...
      .addEventListener("click", () => finish(null), { signal });
      ...
      () => {
        if (withNewPasswordField) {
          ...
          if (!newVal || newVal.length < 12) {
            newPwError.style.display = "";
            newPwInput.focus();
            return;
          }
          if (withPasswordField) {
            const adminVal = passwordInput.value;
            if (!adminVal) {
              passwordError.style.display = "";
              passwordInput.focus();
              return;
            }
            finish({ adminPassword: adminVal, newPassword: newVal });
            return;
          }
          finish({ newPassword: newVal });
          return;
        }
        if (withPasswordField) {
    ...
    if (withNewPasswordField) {
      newPwInput.addEventListener(
        ...
        (e) => {
          if (e.key === "Enter") {
            if (withPasswordField) passwordInput.focus();
...
async function resetAdminUserPassword(id, username) {
  const result = await openAdminModal({
    ...
    message: `Set a new password for "${username}". This will sign them out of all sessions.`,
    confirmText: "Reset Password",
    withPasswordField: true,
    withNewPasswordField: true,
  });
  if (!result) return;
  ...
  try {
    ...
    showAdminToast(`Password reset for ${username}`, "success");
  } catch (error) {
    showAdminToast(`Failed to reset password: ${error.message}`);
  }
}
  ...
  API.getMe()
    .then(async (user) => {
      ...
      try {
        ...
        const activeTheme =
          serverTheme && serverTheme !== "auto" ? serverTheme : localTheme;
